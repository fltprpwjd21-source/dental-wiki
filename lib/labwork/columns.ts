...
export const LABWORK_COLUMNS: LabworkColumn[] = [
  ...
  // 「도착일」 → 「예정일」 → 「완료일」로 왔다 (2026-09-10).
  //
  //   「예정일」로 두면 앞의 J열과 이름이 같아지는데, 두 칸은 하는 일이 전혀 다르다.
  //   이 칸에 값이 들어가면 그 줄은 완료로 처리되어 아래로 내려가고 연한 하늘색이 된다
  //   (저장되는 값이 arrived_on 이고, 도착 체크·정렬·바탕색이 모두 이 칸을 본다).
  //   같은 이름을 달아 두면 앞 칸인 줄 알고 예정일을 적는 순간 그 줄이 완료로 내려간다 —
  //   이름이 동작과 어긋나면 그건 함정이 된다.
  //
  //   「완료일」로 부르면 이름과 동작이 맞는다. 칸을 없애지 않은 이유는,
  //   어제 온 것을 오늘 체크했을 때 날짜를 고칠 자리가 필요하기 때문이다
  //   (체크 버튼은 언제나 오늘로 찍는다).
  { key: "arrived_on", label: "완료일", kind: "date", width: "5rem" },
