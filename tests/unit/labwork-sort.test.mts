...
describe("기공물 줄 세우기", () => {
  ...
  test("안 온 것끼리는 최근 의뢰가 위", () => {
    // 오래된 것을 위에 두면, 새 줄에 오늘 날짜를 적는 순간 맨 아래로 내려가
    // 방금 만든 줄이 눈앞에서 사라진다 (2026-09-10 에 뒤집은 이유).
    const 늦게 = row({ id: "늦게", ordered_on: "2026-09-09" });
    const 먼저 = row({ id: "먼저", ordered_on: "2026-09-02" });
    assert.deepEqual(ids([먼저, 늦게]), ["늦게", "먼저"]);
  });

  test("표 전체가 한 방향이다 — 안 온 것도 완료도 최근이 위", () => {
    // 방향이 중간에 뒤집히면 매번 어느 쪽이 최근인지 다시 생각하게 된다.
    const 안옴옛 = row({ id: "안옴옛", ordered_on: "2026-08-01" });
    const 안옴새 = row({ id: "안옴새", ordered_on: "2026-09-09" });
    const 완료옛 = row({ id: "완료옛", ordered_on: "2026-08-02", arrived_on: "2026-09-01" });
    const 완료새 = row({ id: "완료새", ordered_on: "2026-09-08", arrived_on: "2026-09-09" });
    assert.deepEqual(ids([완료옛, 안옴옛, 완료새, 안옴새]), ["안옴새", "안옴옛", "완료새", "완료옛"]);
  });

  test("새 줄에 오늘 날짜를 적어도 위에 남는다", () => {
    const 오래된 = row({ id: "오래된", ordered_on: "2026-08-01" });
    const 방금적음 = row({ id: "방금적음", ordered_on: "2026-09-10" });
    assert.deepEqual(ids([오래된, 방금적음]), ["방금적음", "오래된"]);
  });
  ...
  test("같은 의뢰일이면 나중에 넣은 줄이 위", () => {
    // 방금 만든 줄이 위에 보여야 이어서 채울 수 있다.
    const 첫째 = row({ id: "첫째", ordered_on: "2026-09-03" });
    const 둘째 = row({ id: "둘째", ordered_on: "2026-09-03" });
    assert.deepEqual(ids([첫째, 둘째]), ["둘째", "첫째"]);
  });
